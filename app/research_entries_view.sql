...
SELECT de.id AS entry_id, de.title, de.description, STRING_AGG((SELECT t.value WHERE t.category = 'dream_content'), ';') AS content_tags,
	   ...
	   de.sleep_hours, de.country, de.state, de.city, CASE WHEN COALESCE(u.country, u.state, u.city) IS NULL THEN NULL 
	   ELSE BOOL_OR(t.value = 'Atypical Location') END AS not_at_home, de.reflection, de.rfln_timestamp, u.username, 
	   EXTRACT(DAY FROM (NOW()-u.birth_date))/365.25 AS user_age, u.gender AS user_gender, u.med_conditions AS user_med_conditions
